...
import { decodeFixed, lampContext, signFrame, verifyFrame } from './crypto.js'
import { LampStore } from './store.js'

export type LampFrame = Record<string, unknown>
export type LampSend = (frame: LampFrame) => boolean
export interface LampResume { serverInstanceId: string; cursor: number }
export interface LampTransportOptions {
  machineId: string; machineName: string; identity: Identity; dataDir: string; serverInstanceId: string
  bind?: string; port?: number; capabilities: string[]; limits?: LampFrame
  resume?: (resume?: LampResume) => { resumed: boolean; cursor: number }
  onRequest: (lampId: string, request: LampFrame, send: LampSend, capabilities: string[]) => void | Promise<void>
  onConnected?: (lampId: string, resume: LampResume | undefined, send: LampSend) => void
  onDisconnected?: (lampId: string) => void
  onRevoked?: (lampId: string) => void
}
export class LampTransportError extends Error {
...
interface Session { ws: WebSocket; lampId: string; pub: string; keys: ReturnType<typeof sessionKeys>; rx: ReplayWindow; tx: number; ready: boolean; capabilities: string[]; resume?: LampResume; challenge: string }
export class LampTransport {
  readonly store: LampStore
  private server?: Server
  private wss?: WebSocketServer
  private opening?: Promise<void>
  private window?: PairWindow
  private pairState: LampFrame = { state: 'idle' }
  ...
  constructor(private readonly options: LampTransportOptions) { this.store = new LampStore(options.dataDir) }
  ...
  private async listen(): Promise<void> {
    ...
    this.opening = new Promise<void>((resolve, reject) => {
      ...
      server.on('upgrade', (req, socket, head) => {
        if (req.url !== '/api/lamp-ws' || req.headers.origin || req.headers.authorization || req.headers.cookie || req.headers['sec-websocket-protocol'] || wss.clients.size >= 16) { socket.destroy(); return }
  ...
  async pairStart(input: { replace?: boolean } = {}): Promise<LampFrame> {
    if (this.window || this.pairingStarting) throw new LampTransportError('BUSY')
    if (this.store.paired() && !input.replace) throw new LampTransportError('ALREADY_PAIRED')
  ...
  pairStatus(): LampFrame { return { ...this.pairState } }
  list(): LampFrame[] {
    return [this.store.paired(), this.store.pending()].filter(r => r !== null).map(r => ({ id: r.id, label: r.label, fingerprint: fingerprint(b64d(r.identityPub)), pairedAt: r.pairedAt, lastSeenAt: r.lastSeenAt, enabled: true, pendingFirstSession: r.pendingFirstSession, online: this.session?.lampId === r.id && this.session.ready }))
  }
  status(): LampFrame { return { listening: !!this.server, bind: this.options.bind ?? '0.0.0.0', port: this.actualPort || this.options.port || 18474, address: this.address(), paired: Number(!!this.store.paired()), sessions: Number(!!this.session?.ready), serverInstanceId: this.options.serverInstanceId, proto: 1 } }
  revoke(id: string): number {
    ...
    for (const s of this.sessions.values()) if (ids.includes(s.lampId)) this.drop(s, 4403, 'REVOKED')
    for (const lampId of ids) { this.requests.delete(lampId); this.options.onRevoked?.(lampId) }
    return count
  }
  send(frame: LampFrame): boolean { return this.session ? this.sendSession(this.session, frame) : false }
  private sendSession(s: Session, frame: LampFrame): boolean {
  ...
  private drop(s: Session, code: number, reason: string): void {
    ...
    if (this.session === s) { this.session = undefined; this.options.onDisconnected?.(s.lampId) }
  ...
  private clear(ws: WebSocket, frame: LampFrame): void { if (ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify(frame)) }
  private deny(ws: WebSocket, code: string, close = 4401): void { this.clear(ws, { type: 'lamp_denied', error: { code, message: code } }); ws.close(close, code) }
  private finishPair(error?: string): void {
    const w = this.window; this.window = undefined
    if (w?.socket && error) this.clear(w.socket, { type: 'lamp_pair_error', error: { code: error, message: error } })
  ...
  private connect(ws: WebSocket): void {
    ...
    ws.on('close', () => {
      ...
      if (s && this.session === s) { this.session = undefined; this.options.onDisconnected?.(s.lampId) }
    ...
    ws.on('message', (raw, binary) => {
      try {
        ...
        this.receive(ws, f as LampFrame)
      } catch { this.deny(ws, 'INVALID_FRAME') }
    })
  }
  private receive(ws: WebSocket, f: LampFrame): void {
    ...
    if (s) {
      ...
      const request = value as LampFrame
      if (request.type !== f.type || request.agentId !== f.agentId) return
      s.rx.commit(env.n)
      if (!s.ready) {
        if (request.type !== 'lamp_finished' || request.challenge !== s.challenge) return
        ...
        this.pairState = { state: 'paired', pendingFirstSession: false, lampLabel: confirmed.label, lampFingerprint: fingerprint(b64d(s.pub)) }
        ...
        this.sendSession(s, { type: 'lamp_ready', serverInstanceId: this.options.serverInstanceId })
        this.options.onConnected?.(s.lampId, s.resume, frame => this.sendSession(s, frame))
        return
      }
      if (this.session !== s) return
      const now = Date.now(), budget = this.requests.get(s.lampId) ?? { tokens: 20, at: now, inFlight: 0 }
      budget.tokens = Math.min(20, budget.tokens + Math.max(0, now - budget.at) / 1000); budget.at = now
      this.requests.set(s.lampId, budget)
      ...
      void Promise.resolve().then(() => { if (s.ready && this.session === s && this.store.find(s.pub)) return this.options.onRequest(s.lampId, request, frame => this.sendSession(s, frame), s.capabilities) })
    ...
    if (f.type === 'lamp_pair_intent' || f.type === 'lamp_pake') { this.pair(ws, f); return }
    if (f.type !== 'lamp_hello') { this.deny(ws, 'UNAUTHORIZED'); return }
    if (f.machineId !== this.options.machineId) { this.deny(ws, 'MACHINE_MISMATCH'); return }
    const row = typeof f.lampId === 'string' ? this.store.find(f.lampId) : null
    if (!row) { this.deny(ws, 'UNKNOWN_LAMP', 4404); return }
    ...
    let resume: LampResume | undefined
    if (f.resume !== undefined) {
      const r = f.resume as LampResume
    ...
    const session: Session = { ws, lampId: row.id, pub: row.identityPub, keys: sessionKeys(eph.priv, peerEph, this.options.machineId, peerEph, eph.pub), rx: new ReplayWindow(), tx: 0, ready: false, capabilities, resume, challenge: randomUUID() }
    const welcome: LampFrame = { type: 'lamp_welcome', proto: 1, machineId: this.options.machineId, machineName: this.options.machineName, ephPub: b64e(eph.pub), serverInstanceId: this.options.serverInstanceId, capabilities, limits: { maxFrameBytes: 65536, maxPromptBytes: 16384, eventBufferSize: 500, dedupeEntries: 512, dedupeTtlMs: 1800000, requestBurst: 20, requestRefillPerSecond: 1, maxConcurrentRequests: 4, ...this.options.limits }, resumed: false, cursor: 0, ...this.options.resume?.(resume), challenge: session.challenge }
  ...
  private pair(ws: WebSocket, f: LampFrame): void {
    const w = this.window
    if (!w || w.expiresAt <= Date.now()) { this.clear(ws, { type: 'lamp_pair_error', error: { code: 'EXPIRED' } }); return }
    try {
      if (f.type === 'lamp_pair_intent') {
        if (w.socket && w.socket !== ws) { this.clear(ws, { type: 'lamp_pair_error', error: { code: 'BUSY' } }); return }
        ...
        if (f.role !== 'lamp' || typeof f.label !== 'string' || f.label.length < 1 || f.label.length > 80 || /[\x00-\x1f\x7f]/.test(f.label)) throw new Error('invalid label')
        w.attempts++; w.socket = ws; w.pairId = f.pairId as string; w.label = f.label
        const start = cpaceStart(cpaceGenerator(w.code, pairId, lampContext(this.options.machineId)))
        w.y = start.y; w.ya = start.Y
        this.pairState = { state: 'running', expiresAt: w.expiresAt }
        this.clear(ws, { type: 'lamp_pair_intent_result', accepted: true, machineId: this.options.machineId, machineName: this.options.machineName, ttl: Math.max(0, Math.floor((w.expiresAt - Date.now()) / 1000)) })
        this.clear(ws, { type: 'lamp_pake', pairId: w.pairId, round: 1, ya: b64e(start.Y) }); return
      }
      if (w.socket !== ws || f.pairId !== w.pairId) throw new Error('no intent')
      const ci = lampContext(this.options.machineId)
      if (f.round === 2 && w.y && w.ya && !w.key) {
        ...
        this.clear(ws, { type: 'lamp_pake', pairId: w.pairId, round: 3, mac: b64e(macTag(kc.adapter, th)), enc: b64e(aeadSeal(w.key, 3, utf8('e2e-id'), utf8(JSON.stringify({ id: b64e(id.pub), sig: b64e(pairBindSig(id.priv, th)) })))) }); return
      }
      if (f.round === 4 && w.key && w.th && typeof f.enc === 'string') {
        ...
        this.clear(ws, { type: 'lamp_pake', pairId: w.pairId, round: 5, ok: true, fingerprint: fingerprint(this.options.identity.pub), lampId: row.id })
        this.finishPair(); this.pairState = { state: 'paired', pendingFirstSession: true, lampLabel: row.label, lampFingerprint: fingerprint(pub) }; return
      }
      throw new Error('unexpected round')
    } catch {
      this.clear(ws, { type: 'lamp_pair_error', error: { code: 'CODE_MISMATCH' } })
