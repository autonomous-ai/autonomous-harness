...
const dirs: string[] = []
function fixture() { const dir = mkdtempSync(join(tmpdir(), 'device-store-')); dirs.push(dir); return dir }
afterEach(() => { for (const dir of dirs.splice(0)) rmSync(dir, { recursive: true, force: true }) })
describe('device durable private trust', () => {
  it('preserves an incumbent until a candidate confirms and expires pending across restart', () => {
    const dir = fixture(), store = new AutonomousDeviceStore(dir), old = b64e(newIdentity().pub), next = b64e(newIdentity().pub)
    store.stage(old, 'Old'); store.confirm(old); store.stage(next, 'Next')
    const restored = new AutonomousDeviceStore(dir)
    expect(restored.paired()?.id).toBe(old); expect(restored.pending()?.id).toBe(next)
    restored.expire(Date.now() + 300_001)
    expect(new AutonomousDeviceStore(dir).pending()).toBeNull(); expect(new AutonomousDeviceStore(dir).paired()?.id).toBe(old)
  ...
  it('refuses a symlink trust file without modifying the target', () => {
    const dir = fixture(); new AutonomousDeviceStore(dir)
    const target = join(dir, 'target'); writeFileSync(target, 'do not change', { mode: 0o600 })
    symlinkSync(target, join(dir, 'e2e', 'autonomous-devices.json'))
    expect(() => new AutonomousDeviceStore(dir)).toThrow()
  ...
  it('refuses a symlink directory and group-writable existing state', () => {
    ...
    expect(() => new AutonomousDeviceStore(dir)).toThrow(/writable/)
  })
  it('refuses an unsafe replacement target on an existing loaded store', () => {
    const dir = fixture(), store = new AutonomousDeviceStore(dir), pub = b64e(newIdentity().pub)
    store.stage(pub, 'Autonomous device'); chmodSync(join(dir, 'e2e', 'autonomous-devices.json'), 0o666)
