...
async function pair(server: AutonomousDeviceTransport, code: string, client: Identity) {
  ...
  server.pairStart({ code: 'K7P4X9', pairId, replace: true })
...
describe('device LAN trust and encryption', () => {
  it('waits for the device code entered locally and refuses stale pair targets', async () => {
    const { server } = fixture()
    const window = await server.pairListen()
    expect(window.code).toBeUndefined()
    expect(window.state).toBe('listening')
    const { ws, next } = await socket(server), pairId = b64e(newPairId())
    ws.send(JSON.stringify({ type: 'autonomous_device_pair_intent', role: 'autonomous-device', label: 'Kitchen', pairId }))
    expect((await next()).accepted).toBe(true)
    expect(server.pairStatus()).toMatchObject({ state: 'waiting', pairId, deviceLabel: 'Kitchen' })
    expect(() => server.pairStart({ code: 'K7P4X9', pairId: b64e(newPairId()) })).toThrow('STALE_PAIR')
    expect(server.pairStatus().state).toBe('waiting')
    const status = server.pairStart({ code: 'K7P4X9', pairId })
    expect(status.code).toBeUndefined()
    expect((await next()).round).toBe(1)
    expect(() => server.pairStart({ code: 'K7P4X9', pairId })).toThrow('BUSY')
    server.pairCancel()
  ...
  it('pairs, persists private trust, dispatches only encrypted untampered requests, and revokes', async () => {
    ...
    const window = await server.pairListen(); await pair(server, 'K7P4X9', client)
    expect(server.store.paired()).toBeNull()
  ...
  it('preserves incumbent through wrong code and replacement until encrypted finished', async () => {
    ...
    let window = await server.pairListen(); await pair(server, 'K7P4X9', old)
    const incumbent = await handshake(server, old, identity)
    await expect(server.pairListen()).rejects.toMatchObject({ code: 'ALREADY_PAIRED' })
    window = await server.pairListen({ replace: true })
    ...
    window = await server.pairListen({ replace: true }); await pair(server, 'K7P4X9', replacement)
