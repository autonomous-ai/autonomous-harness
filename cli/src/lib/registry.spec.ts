...
/**
 * The transcript gate confines a session file to its engine's root — it is what stops a session id from a
 * request being resolved into an arbitrary path. Per-agent muse roots widen WHICH root is legitimate, so
 * these tests exist to prove the gate still shuts.
 */
describe('transcript gate with per-agent muse roots', () => {
  let museA = ''
  let museB = ''

  beforeEach(() => {
    dataDir = mkdtempSync(join(tmpdir(), 'adapter-registry-'))
    museA = mkdtempSync(join(tmpdir(), 'muse-a-'))
    museB = mkdtempSync(join(tmpdir(), 'muse-b-'))
  })
  afterEach(() => {
    for (const d of [dataDir, museA, museB]) rmSync(d, { recursive: true, force: true })
  })

  function writeSession(home: string, id: string): string {
  ...
  it('accepts a transcript under the agent\'s own root', async () => {
    process.env.MUSE_HOME = museA
    const { validTranscriptPath } = await loadRegistryModule()
    expect(validTranscriptPath('muse', writeSession(museB, 's-b'), museB)).toBe(true)
  })

  it('REFUSES a transcript belonging to a different agent\'s root', async () => {
    // Without this the widening would be a back door: any agent could name any other agent's file.
    process.env.MUSE_HOME = museA
    const { validTranscriptPath } = await loadRegistryModule()
    expect(validTranscriptPath('muse', writeSession(museB, 's-b'), museA)).toBe(false)
  })

  it('falls back to the daemon default when an agent declares no root', async () => {
    // Old launchers send nothing, and their agents must keep working exactly as before.
    process.env.MUSE_HOME = museA
    const { validTranscriptPath } = await loadRegistryModule()
    expect(validTranscriptPath('muse', writeSession(museA, 's-a'))).toBe(true)
    expect(validTranscriptPath('muse', writeSession(museB, 's-b2'))).toBe(false)
  })
})
