...
- `toolchain/setup.sh` — one `node_modules` (pinned in `package.json`) that every workspace links to,
  ...
  commit in `VERSIONS` — not copied into this repository, because
...
This repository is the Harness wrapper — the manifest, the pane script, the template, the toolchain,
...
repository and point the registry entry at it. Until then: bugs in Remotion belong upstream, bugs in
