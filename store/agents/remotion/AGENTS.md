...
opened the **Remotion Studio pane**: Studio on this project, playing the composition and reloading
as you save. You never start Studio, never print a URL, never open a browser.
...
3. **Render when a beat is done**: `$REMOTION render Main out/main.mp4` (minutes for a long video;
   `--frames=0-90` to check a section). The verdict names the newest render and flags a stale one.
