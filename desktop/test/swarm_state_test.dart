...
AppNotifier createApp({MemoryStore? store}) {
  final app = AppNotifier(config: AppConfig.dev, authSession: AuthSession(), configStore: null,
    paneLayoutStore: store == null ? null : PaneLayoutStore(storage: store))..hasNavigationRail = false;
  const machine = Machine(machineId: 'm', authMode: MachineAuthMode.remote, name: 'Test host');
  app.machines = [machine];
  app.machineStates['m'] = MachineState(machine)..nodeOnline = false
    ..agentLoadStatus = AgentLoadStatus.loaded
    ..agents = [for (var i = 0; i < 70; i++) Agent(id: 'a$i', name: 'Agent $i', engine: 'codex', terminalAvailable: true)];
  return app;
}
void main() {
  test('shared memberships own one controller and close only the final stream', () async {
    final app = createApp();
    final sent = <String>[];
    final session = TerminalSession(machineId: 'm', agentId: 'a0', agentName: 'A', engineId: 'codex',
      send: (type, payload) async { sent.add(type); return true; }, sendBinary: (_) async => true)
      ..status = TerminalSessionStatus.controlling..streamId = 'shared';
  ...
  test('seeding records every membership before any tab switch and exceeds nine', () async {
    final app = createApp();
    final first = app.activeSwarm;
    final work = app.seedSwarm('Fleet', [for (var i = 0; i < 20; i++) (machineId: 'm', agentId: 'a$i')]);
    app.newSwarm();
    await work;
  ...
  test('starter capacity is explicit and never evicts selected agents', () async {
    final app = createApp();
    await app.seedSwarm('Large fleet', [for (var i = 0; i < 70; i++) (machineId: 'm', agentId: 'a$i')]);
  ...
  test('all swarm intent, focus, zoom, wallpaper and shared identity restore offline', () async {
    ...
    expect(restored.swarms.first.panes.first, same(restored.swarms.last.panes.single));
  ...
  test('new tab ids cannot collide after reorder and gaps in saved ids', () async {
    final app = createApp();
    app.newSwarm(); app.newSwarm(); app.newSwarm();
    await app.closeSwarm('swarm-2');
    app.reorderSwarm('swarm-4', 0);
    app.newSwarm(); app.newSwarm();
    expect(app.swarms.map((s) => s.id).toSet().length, app.swarms.length);
    while (app.swarms.length > 1) { await app.closeSwarm(app.swarms.first.id); }
    await app.closeSwarm(app.activeSwarmId);
    expect(app.swarms.length, 1); expect(app.panes, isEmpty);
    app.dispose();
  });

  test('focus changes follow zoom and wrap without a hidden rail', () async {
    final app = createApp();
    await app.addAgentToSwarm('m', 'a0'); await app.addAgentToSwarm('m', 'a1');
    app.toggleZoomPane();
    app.focusPane(app.panes.first.id);
  ...
  test('same remote groups across machines but matching folder names do not', () {
    final app = createApp();
    ...
    remote.agents = [const Agent(id: 'a', name: 'A', project: AgentProject(name: 'app', cwd: '/one/app', remote: 'github.com/org/app')),
      const Agent(id: 'b', name: 'B', project: AgentProject(name: 'private', cwd: '/one/private'))];
    app.machineStates['n']!.agents = [const Agent(id: 'c', name: 'C', project: AgentProject(name: 'app', cwd: '/two/app', remote: 'github.com/org/app')),
      const Agent(id: 'd', name: 'D', project: AgentProject(name: 'private', cwd: '/one/private'))];
    final groups = swarmProjects(app, []);
    expect(groups.length, 3);
    expect(groups.firstWhere((g) => g.name == 'app').agents.length, 2);
    expect(Agent.fromJson({'id': 'old', 'name': 'Older daemon'}).project, isNull);
    app.dispose();
  });
}
