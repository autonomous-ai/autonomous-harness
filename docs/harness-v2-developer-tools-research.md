...
| **Improve first** | `Cmd+P` currently opens **Add agent** in the Swarm screen | Make “jump to existing work” distinct from “add another view.” The present picker primarily changes membership. Add fuzzy ranking, recent destinations and Swarm matches to a navigation interface; retain an explicit Add command. |
...
| **Remove avoidable waiting** | Settings route transitions and section cross-fades | The current source has 170/120 ms route transitions and a 200 ms section transition. Prefer immediate navigation, with animation reserved for feedback that helps understanding. No need for another settings feature. |
