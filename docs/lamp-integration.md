# Lamp ↔ Harness CLI: implemented protocol v1
...
Lamp connects to a dedicated LAN WebSocket on the CLI. Desktop manages pairing through the
existing authenticated loopback hook API. Desktop need not stay open. No cloud/backend changes,
lamp SSO token, raw terminal access, shell execution, agent creation/deletion, remote-machine
...
Implementation: `cli/src/lib/lamp/{transport,crypto,store,service,localApi,command}.ts` and wiring in
...
Default bind `0.0.0.0`, port `18474`; configure `HARNESS_LAMP_BIND` and `HARNESS_LAMP_PORT`.
`HARNESS_LAMP_IFACE` and mDNS are not implemented. Pairing returns a manually entered
...
LAN URL: `ws://<host>:18474/api/lamp-ws`. No TLS; authenticated application encryption supplies
...
|---|---|---|
| POST `/api/lamp/pair/start` | `{}` or `{replace:true}` | `{code,expiresAt,machineId,machineName,address,fingerprint}` |
| POST `/api/lamp/pair/cancel` | `{}` | `{cancelled:true}` |
| GET `/api/lamp/pair/status` | — | `{state,expiresAt?,pendingFirstSession?,lampLabel?,lampFingerprint?,error?}` |
| GET `/api/lamp/list` | — | `{lamps:[{id,label,fingerprint,pairedAt,lastSeenAt,enabled,online,pendingFirstSession}]}` |
| POST `/api/lamp/revoke` | `{id}` or `{all:true}` | `{revoked:number}` |
| GET `/api/lamp/status` | — | `{listening,bind,port,address,paired,sessions,serverInstanceId,proto:1}` |
| GET `/api/lamp/receipt?lampId=…&idempotencyKey=…` | — | `{receipt:Receipt|null}` |

`id` and `lampId` are the **canonical base64 32-byte Ed25519 public key**, not fingerprint.
...
## Pairing and single-lamp trust

Opening a local 60-second pairing window is the human authorization. CLI displays a six-character
code. Lamp receives address and code. At most three PAKE attempts per window; a new window must
...
The CLI is CPace initiator `a`; lamp is responder `b`. Pair ID is 16 random bytes, canonical base64.
Only cryptographic handshake material and display metadata are cleartext.

```
LAMP_CI = autonomous-e2e-pair|agent:<machineId>|a:adapter|b:lamp
```

1. Lamp sends `{type:"lamp_pair_intent",pairId,role:"lamp",label}`.
2. CLI replies `{type:"lamp_pair_intent_result",accepted:true,machineId,machineName,ttl}`,
   then `{type:"lamp_pake",pairId,round:1,ya}`. The received machine ID is provisional until PAKE
   authentication succeeds; altering it changes the channel binding and fails authentication.
3. Lamp derives generator from code, pair ID and CI, creates `(y,Yb)` and computes shared K,
   `isk=cpaceISK(sid,K,Ya,Yb)`, `th=transcriptHash(sid,CI,Ya,Yb)`, `kc=kcKeys(isk,CI)`.
   Sends `{type:"lamp_pake",pairId,round:2,yb,mac:base64(macTag(kc.web,th))}`.
4. CLI verifies MAC and returns round 3 with `mac=base64(macTag(kc.adapter,th))` and encrypted
   identity `enc`. Lamp verifies MAC, decrypts identity, verifies pair binding and retains a
   provisional CLI pin for reconnect recovery.
5. Lamp returns round 4 with its encrypted identity and pair binding signature. CLI durably stages
   the candidate, preserving incumbent, then replies round 5 `{ok:true,fingerprint,lampId}`.
6. Lamp completes the session handshake below. **Only encrypted `lamp_finished` promotes the
   candidate and replaces the incumbent.** A failed/expired/cancelled attempt cannot remove the old
   lamp. Losing round 5 is recoverable using the provisional pin and a normal signed session.
...
Pair errors use `{type:"lamp_pair_error",error:{code}}`; current state machine returns
...
Trust file `${ADAPTER_DATA_DIR}/e2e/lamps.json` is atomically replaced, mode 0600 in directory 0700:
...
history for the old lamp. It cannot undo an already injected prompt.
...
```json
{"type":"lamp_hello","proto":1,"lampId":"<base64 identity pub>","ephPub":"<base64 X25519 pub>","machineId":"machine","capabilities":["agents.list","turn.send","turn.stop","status","recap","question.answer","receipt.get"],"client":{"name":"autonomous-lamp","version":"1"},"resume":{"serverInstanceId":"previous","cursor":4},"sig":"<base64>"}
```
...
helloMessage = lvCat("lamp-hello-v1", machineId, H(hello))
welcomeMessage = lvCat("lamp-welcome-v1", machineId, H(welcome), lampEphemeralPubBytes)
sig = base64(Ed25519.sign(identityPrivate, message))
```

CLI verifies against stored pin. Unknown identity → `UNKNOWN_LAMP`; wrong signature →
`UNAUTHORIZED`; signed unsupported proto → `PROTO_UNSUPPORTED`.

Welcome is `{type:"lamp_welcome",proto:1,machineId,machineName,ephPub,serverInstanceId,
...
ordering `(lampEphPub,cliEphPub)` on both sides. Lamp verifies the pinned CLI signature before
using keys. Then lamp sends encrypted `{type:"lamp_finished",challenge}`. CLI promotes trust,
closes the older session and sends encrypted `{type:"lamp_ready",serverInstanceId}`, followed by
replay/resync. A replayed signed hello without ephemeral possession cannot disconnect a live lamp.
...
counter followed by four zero bytes. Directional counters start at zero; `lamp_finished` uses
client counter zero and `lamp_ready` uses server counter zero. Replay window is 4096 counters;
...
| 4403 | Revoked |
| 4404 | Unknown lamp |
| 4408 | Same lamp superseded: old socket must not reconnect |
| 4409 | Unsupported protocol |
| 4410 | Different lamp replaced incumbent |
...
Untrusted `lamp_denied` cleartext has `{error:{code,message}}`. OS must not let a network attacker
...
```json
{"idempotencyKey":"lamp-1","deliveryId":"<uuid>","operation":"turn.send","state":"queued","machineId":"machine","agentId":"agent","serverInstanceId":"<uuid>","turnId":null,"error":null,"at":1757302040000}
```
...
Dedupe is reserved synchronously before dispatch, keyed `(lampId,idempotencyKey)`, comparing all
...
across restart. Revoke clears the old lamp's receipts and event replay history.

Events are encrypted full objects:

```json
{"type":"event","eventId":1,"serverInstanceId":"<uuid>","machineId":"machine","agentId":"agent","kind":"receipt.updated","payload":{"receipt":{},"idempotencyKey":"lamp-1"}}
```
...
Only lamp-origin turns with known correlation include `idempotencyKey`/`turnId`.
...
`cli/src/lib/lamp/vectors/lamp-protocol.json` contains **public test keys** and deterministic
CPace generator/scalars/shared secret/ISK/MACs, identity ciphertext, signed session messages,
X25519 session keys, encrypted `lamp_finished` (client counter 0), encrypted `lamp_ready`
...
```sh
./node_modules/.bin/tsx src/lib/lamp/vectors/generate.ts
```

Generation is not a test. Real WebSocket pairing/reconnect/revoke/tamper tests and service/local
API tests are authored; execution is deferred while the three repositories' code is completed.
Before release run CLI typecheck/tests/build and cross-language fixture assertions, then an
explicitly authorized physical-device pairing/voice flow. No device was deployed or paired here.
