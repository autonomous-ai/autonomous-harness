...
void main() {
  ...
  group('staging: the words go into the prompt before they are sent', () {
    final prompt = Object();

    test('done talking writes the words, and nothing is sent', () async {
      backend.replies.add('run the tests');
      final written = <String>[];
      await voice.startListening();
      await voice.stage(prompt, (text) {
        written.add(text);
        return true;
      });

      expect(written, ['run the tests']);
      expect(voice.staged, 'run the tests');
      expect(voice.isStagedIn(prompt), isTrue);
      expect(voice.transcript, isEmpty);
      expect(voice.isIdle, isFalse);
    });

    test('send presses Return and puts the mic at rest', () async {
      backend.replies.add('run the tests');
      await voice.startListening();
      await voice.stage(prompt, (_) => true);
      var enters = 0;
      voice.sendStaged(() {
        enters++;
        return true;
      ...
      expect(enters, 1);
      expect(voice.staged, isNull);
      expect(voice.isIdle, isTrue);
    });

    test('discarding hands the words back to be erased', () async {
      backend.replies.add('never mind');
      await voice.startListening();
      await voice.stage(prompt, (_) => true);
      final erased = <String>[];
      voice.discardStaged(erased.add);

      expect(erased, ['never mind']);
      expect(voice.isIdle, isTrue);
    });

    test('a prompt that will not take them keeps the words here', () async {
      backend.replies.add('run the tests');
      await voice.startListening();
      await voice.stage(prompt, (_) => false);
    ...
    test('done while the mic is still opening calls it off', () async {
      recorder.pendingPermission = Completer<bool>();
      final starting = voice.startListening();
      final written = <String>[];
      await voice.stage(prompt, (text) {
        written.add(text);
        return true;
    ...
    test(
      'the keyboard taking over forgets them, and leaves them typed',
      () async {
        backend.replies.add('almost right');
        await voice.startListening();
        await voice.stage(prompt, (_) => true);
