 ...
 *
 * This file is the shared vocabulary between every collector (remote CLI, machine-node, provider)
 * and the backend ingest. It is duplicated verbatim in `apps/machine-node/api/src/types/analytics.ts`
 * and in `autonomous-harness/cli` — there is no shared package in this monorepo, so the copies are
 * kept in sync by the conformance tests, not by the module system.
 *
